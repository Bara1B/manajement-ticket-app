...
@section('content')
    ...
    <div class="container mx-auto p-10">
        <h1 class="text-3xl font-semibold mb-6">Edit Tipe Tiket</h1>
        
        <div class="bg-white p-8 rounded-box shadow-xs max-w-2xl">
            <form method="POST" action="{{ route('admin.tipe-tiket.update', $tipeTiket->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="form-control w-full mb-6">
                    ...
                    <textarea 
                        ...
                        name="keterangan"
                        rows="4"
                    >{{ old('keterangan', $tipeTiket->keterangan) }}</textarea>
                    @error('keterangan')
                        <label class="label">
                            <span class="label-text-alt text-red-500">{{ $message }}</span>
                        </label>
